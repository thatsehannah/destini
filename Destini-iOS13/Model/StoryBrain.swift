...
struct StoryBrain{
    ...
    func getStoryChoices() -> [String] {
        let choices = [stories[position].choice1, stories[position].choice2]
        return choices
    }
    
    mutating func nextQuestion(_ userChoice: String) {
        if (userChoice == stories[position].choice1){
            position = stories[position].choice1Destination
        } else {
            position = stories[position].choice2Destination
        }
    }
}
